...
if (isset($_POST['submit'])) {
    ...
    $lenguajes = capitalizeFirstLetter($_POST['lenguajes']);
    $capacidad = $_POST['capacidad'];

    // Prepare the SQL statement
    $sql = "INSERT INTO proyecto (titulo, descripcion, lenguajes, capacidad) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        echo "Error: failed to prepare the SQL statement";
    } else {
        // Bind the parameters to the placeholders in the SQL statement
        mysqli_stmt_bind_param($stmt, "sssi", $titulo, $descripcion, $lenguajes, $capacidad);
